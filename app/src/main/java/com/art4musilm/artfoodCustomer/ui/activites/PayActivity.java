...
public class PayActivity extends AppCompatActivity implements SessionDelegate {
    ...
    private void configureApp() {
        GoSellSDK.init(this, "sk_live_fGBrPdweNVyaH9bXA0sUIgOZ", "com.art4musilm.artfoodCustomer");  // to be replaced by merchant
    ...
    private void configureSDKSession() {
        ...
        sdkSession.setAmount(new BigDecimal(getIntent().getExtras().getString("priceValue")));  //** Required **
...
//        saveCustomerRefInSession(charge);
        configureSDKSession();
        Log.e("transactionId", charge.getId() + "....." + charge.getResponse().getMessage());
    ...
    public void paymentFailed(@Nullable Charge charge) {
        ...
        Toast.makeText(this, "" + charge.getStatus() + "..." + charge.getDescription(), Toast.LENGTH_SHORT).show();
...
//        showDialog(authorize.getId(), authorize.getResponse().getMessage(), company.tap.gosellapi.R.drawable.icon_failed);
        Toast.makeText(this, "" + authorize.getStatus() + "..." + authorize.getResponse().getMessage(), Toast.LENGTH_SHORT).show();
    }
...
//                    Intent intent = new Intent(PayActivity.this, PaymentWaysActivity.class);
//                    intent.putExtra("from", "payment");
//                    startActivity(intent);
//                    finish();
                } else {
                    Intent intent = new Intent(PayActivity.this, InvoicesActivity.class);
                    intent.putExtra("from", "payment");
                    intent.putExtra("priceValue", String.valueOf(getIntent().getExtras().getString("priceValue")));
                    startActivity(intent);
                    finish();
                }
